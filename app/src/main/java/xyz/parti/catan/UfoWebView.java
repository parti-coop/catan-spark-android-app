...
public class UfoWebView {
  ...
  private WebChromeClient m_chromeClient = new WebChromeClient() {
    ...
    public boolean onShowFileChooser(WebView mWebView, final ValueCallback<Uri[]> filePathCallback,
        WebChromeClient.FileChooserParams fileChooserParams) {
      if (m_activity == null) {
        return false;
      }

      if (PackageManager.PERMISSION_GRANTED == ContextCompat.checkSelfPermission(m_activity,
    ...
    private boolean showMultiFileChooser(ValueCallback<Uri[]> filePathCallback) {
      if (m_activity == null || filePathCallback == null) {
        return false;
      }
    ...
    // openFileChooser for Android 4.1 (API level 16) up to Android 4.3 (API level
    // 18)
    ...
    // openFileChooser for Android 3.0 (API level 11) up to Android 4.0 (API level
    // 15)
    @SuppressWarnings("unused")
    ...
    // openFileChooser for Android 2.2 (API level 8) up to Android 2.3 (API level
    // 10)
    @SuppressWarnings("unused")
  ...
  private WebViewClient m_webClient = new WebViewClient() {
    ...
    /**
     * @return True if the host application wants to leave the current WebView and
